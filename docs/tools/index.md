...
When a tool requires confirmation, you'll see several options:

- **Yes, allow once:** Execute the operation this one time
- **Allow always:** Trust this tool/operation for the session
- **Edit request (add feedback):** Provide guidance to redirect the AI's approach without fully canceling
- **No:** Cancel the operation

The **Edit request** option is particularly useful when the AI's approach is close but needs adjustment. See [Edit Request with Feedback](../features/edit-request-feedback.md) for more details.
