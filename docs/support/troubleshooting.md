...
- **Error: `JavaScript heap out of memory` or `Allocation failed - heap out of memory`**
  - **Cause:** The Node.js process ran out of memory. This can happen during long sessions, when processing large files, or with complex security operations.
  - **Solution:**
    1. **Restart the CLI** - Memory accumulates over long sessions. Restarting clears the heap.
    2. **Increase memory limit** - Run with more memory allocation:

       ```bash
       # Default 8GB (recommended)
       npm start

       # High memory mode (16GB) for intensive operations
    ...
    3. **Use low memory mode** for constrained systems:
       ```bash
       npm run start:lowmem  # 4GB limit
       ```
    4. **Check available system memory** - Ensure your system has enough RAM. DarkCoder works best with at least 8GB of system RAM.
    5. **Avoid processing very large files** - Split large operations into smaller batches.
