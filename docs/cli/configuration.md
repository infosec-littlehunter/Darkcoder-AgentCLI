...
- **`model.maxOutputTokens`** (number):
  - **Description:** Maximum number of tokens the LLM can generate in a single response. Lower values reduce memory usage and response time, which can help with system lag on lower-memory systems. Set to 0 or leave undefined to use the model's default limit.
  - **Default:** `undefined` (uses model default)
  - **Examples:**
    - `2048` - Limit responses to ~2K tokens
    - `4096` - Limit responses to ~4K tokens
    - `8192` - Limit responses to ~8K tokens

- **`model.conciseMode`** (boolean):
  - **Description:** Enable concise response mode. When enabled, the LLM will provide shorter, more focused responses without unsolicited suggestions, lengthy explanations, or "you might also want to consider" additions. This significantly reduces memory usage and improves responsiveness on systems with limited resources.
  - **Default:** `false`
  - **Example:**
    ```json
    {
...
- **`tools.approvalMode`** (string):
  - **Description:** Sets the default approval mode for tool usage. Accepted values are:
    ...
    - `default`: Require approval before file edits or shell commands run. When prompted for confirmation, you can also use the "Edit request (add feedback)" option to redirect the AI with custom guidance.
    - `auto-edit`: Automatically approve file edits.
    - `yolo`: Automatically approve all tool calls.
  - **Default:** `default`
  - **Note:** In `default` mode, the "Edit request" confirmation option allows you to provide feedback to redirect the AI's approach. See [Edit Request with Feedback](../features/edit-request-feedback.md).
