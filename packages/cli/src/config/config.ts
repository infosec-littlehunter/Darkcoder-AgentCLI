...
import { findProviderByModelId } from '../ui/models/aiProviders.js';

/**
 * Get API key for a model by checking:
 * 1. CLI arg (--openai-api-key)
 * 2. Provider-specific env var (e.g., ANTHROPIC_API_KEY)
 * 3. Generic OPENAI_API_KEY env var
 * 4. Provider-specific settings (security.auth.providers[providerId].apiKey)
 * 5. Generic settings (security.auth.apiKey)
 */
function getApiKeyForModel(
  model: string | undefined,
  argv: { openaiApiKey?: string },
  settings: Settings,
): string | undefined {
  // 1. CLI arg takes precedence
  if (argv.openaiApiKey) {
    return argv.openaiApiKey;
  }

  // Find provider for this model
  const provider = model ? findProviderByModelId(model) : undefined;

  // Also try to detect provider by model name patterns
  let providerId = provider?.id;
  let providerEnvKey = provider?.envKeyName;

  if (!providerId && model) {
    // Fallback detection by model name patterns
    ...
    if (
      ...
      providerId = 'deepseek';
      providerEnvKey = 'DEEPSEEK_API_KEY';
  ...
  // 3. OPENROUTER_API_KEY - works for any model via OpenRouter
  if (process.env['OPENROUTER_API_KEY']) {
    return process.env['OPENROUTER_API_KEY'];
  }

  // 4. Generic OPENAI_API_KEY or GEMINI_API_KEY
  ...
  const providers = settings.security?.auth?.providers as
  ...
  // 6. Generic settings apiKey
  return settings.security?.auth?.apiKey;
}

/**
 * Get base URL for a model by checking:
 * 1. CLI arg (--openai-base-url)
 * 2. OPENAI_BASE_URL env var
 * 3. Provider-specific settings (security.auth.providers[providerId].baseUrl)
 * 4. Provider default base URL from AI_PROVIDERS
 * 5. Generic settings (security.auth.baseUrl)
 * 6. Provider preset baseUrl
 */
function getBaseUrlForModel(
  model: string | undefined,
  argv: { openaiBaseUrl?: string },
  settings: Settings,
  presetBaseUrl: string | undefined,
): string | undefined {
  // 1. CLI arg takes precedence
  if (argv.openaiBaseUrl) {
    return argv.openaiBaseUrl;
  }
  ...
  // Also try to detect provider by model name patterns
  let providerId = provider?.id;
  let providerBaseUrl = provider?.baseUrl;
  ...
  if (!providerId && model) {
    ...
    if (
      ...
      providerId = 'anthropic';
      // Note: Anthropic's direct API doesn't support OpenAI-compatible format
      // We'll set the base URL but the error handler will provide guidance if it fails
      // Users should use OpenRouter for Claude via OpenAI-compatible API
      providerBaseUrl = 'https://api.anthropic.com/v1';

      // Check if user has OPENROUTER_API_KEY - if so, use OpenRouter instead
      if (
  ...
  // 3. Provider-specific settings baseUrl
  const providers = settings.security?.auth?.providers as
    | Record<string, { baseUrl?: string }>
    | undefined;
  if (providerId && providers?.[providerId]?.baseUrl) {
  ...
  // 5. If OPENROUTER_API_KEY is set but no provider matched, use OpenRouter
  if (process.env['OPENROUTER_API_KEY']) {
    return 'https://openrouter.ai/api/v1';
  }

  // 6. Generic settings baseUrl
  if (settings.security?.auth?.baseUrl) {
    return settings.security.auth.baseUrl;
  }

  // 7. Provider preset baseUrl
...
export async function parseArguments(settings: Settings): Promise<CliArgs> {
    ...
    .command('$0 [query..]', 'Launch Qwen Code CLI', (yargsInstance: Argv) =>
      yargsInstance
        ...
        .option('disable-tools', {
          type: 'boolean',
          description:
            'Disable tool/function calling. Use this for models that do not support tools (e.g., some free models on OpenRouter).',
          default: false,
        })
        .deprecateOption(
          'show-memory-usage',
  ...
  completionCommand(yargsInstance);
  ...
  // Handle case where MCP/extensions/completion subcommands are executed - they should exit the process
  // and not return to main CLI logic
  if (
    result._.length > 0 &&
    (result._[0] === 'mcp' ||
      result._[0] === 'extensions' ||
      result._[0] === 'completion')
  ) {
    // These commands handle their own execution and process exit
    process.exit(0);
  }

  // DARKCODER: Ignore positional arguments completely
  // Just start interactive mode - user will chat naturally
  // No binary path processing, no one-shot mode
...
export async function loadCliConfig(
  ...
  // Check for provider preset (e.g., OPENAI_PROVIDER=gemini)
  const providerPreset = process.env['OPENAI_PROVIDER']?.toLowerCase();
  const preset = providerPreset ? PROVIDER_PRESETS[providerPreset] : undefined;
  ...
  const resolvedBaseUrl = getBaseUrlForModel(
    resolvedModel,
    argv,
    settings,
    preset?.baseUrl,
  ...
  return new Config({
    ...
    generationConfig: {
      ...
      model: resolvedModel,
      apiKey: getApiKeyForModel(resolvedModel, argv, settings),
      ...
      disableTools: argv.disableTools ?? settings.model?.disableTools ?? false,
      // Add maxOutputTokens to samplingParams if specified
      samplingParams: {
        ...((
        ...
        ...(settings.model?.maxOutputTokens
          ? { max_tokens: settings.model.maxOutputTokens }
          : {}),
      },
    },
    // Concise mode setting for limiting verbose responses
    conciseMode: settings.model?.conciseMode ?? false,
    ...
    // OSINT API keys from settings
    censysApiKey: settings.advanced?.censysApiKey,
    censysApiId: settings.advanced?.censysApiId,
    censysApiSecret: settings.advanced?.censysApiSecret,
    urlscanApiKey: settings.advanced?.urlscanApiKey,
