...
import { AuthType, OutputFormat } from '@darkcoder/darkcoder-core';
...
function getAuthTypeFromEnv(): AuthType | undefined {
  // Check for any OpenAI-compatible API keys (OPENAI_API_KEY, ANTHROPIC_API_KEY, DEEPSEEK_API_KEY, etc.)
  if (
...
export async function validateNonInteractiveAuth(
  ...
  try {
    ...
    if (!effectiveAuthType) {
      const message = `Please set an Auth method in your ${USER_SETTINGS_PATH} or specify one of the following environment variables before running: ANTHROPIC_API_KEY, OPENAI_API_KEY, DEEPSEEK_API_KEY, OPENROUTER_API_KEY, DASHSCOPE_API_KEY, or QWEN_OAUTH`;
      throw new Error(message);
    }
