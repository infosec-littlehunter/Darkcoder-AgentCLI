...
import { tildeifyPath } from '@darkcoder/darkcoder-core';
...
vi.mock('@darkcoder/darkcoder-core', async (importOriginal) => {
  const original =
    await importOriginal<typeof import('@darkcoder/darkcoder-core')>();
  return {
    ...original,
...
describe('<Footer />', () => {
  ...
  it('does not display context percentage (disabled)', () => {
    const { lastFrame } = renderWithWidth(120, createMockUIState());
    expect(lastFrame()).toContain(defaultProps.model);
    expect(lastFrame()).not.toMatch(/\(\d+% context left\)/);
  });

  it('does not display abbreviated context percentage (disabled)', () => {
