...
export async function checkForUpdates(): Promise<UpdateObject | null> {
  try {
    ...
    // Suppress warning for PackageNotFoundError (local development)
    if (e instanceof Error && e.message.includes('PackageNotFoundError')) {
      // Silently ignore - package is not published to npm
      return null;
    }
    // Only log non-PackageNotFoundError failures in debug mode
