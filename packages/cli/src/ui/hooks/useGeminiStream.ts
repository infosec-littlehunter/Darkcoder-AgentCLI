...
export const useGeminiStream = (
      ...
      return addItem(
        {
          type: 'info',
          text:
            `Context auto-compressed for ${config.getModel()} ` +
            `(${eventValue?.originalTokenCount ?? '?'} → ${eventValue?.newTokenCount ?? '?'} tokens).`,
        },
        Date.now(),
      );
    },
  ...
  const handleMaxSessionTurnsEvent = useCallback(() => {
    // Turn limit enforcement disabled - this handler should not be triggered
    // If reached, it means there's a legacy code path still emitting this event
    console.debug(
      'MaxSessionTurns event received but turn limits are disabled',
    );
  }, []);
  ...
  const handleSessionTokenLimitExceededEvent = useCallback(
    async (value: {
      ...
      // Auto-save task summary to project memory if available
      if (value.taskSummary) {
        try {
          const fs = await import('fs/promises');
          const path = await import('path');
          const memoryPath = path.default.join(process.cwd(), 'QWEN.md');
          let existing = '';
          try {
            existing = await fs.default.readFile(memoryPath, 'utf-8');
          } catch {
            // File doesn't exist yet
          }
          // Append task summary with timestamp
          const timestamp = new Date().toISOString().split('T')[0];
          const newContent = `${existing}\n\n---\n${timestamp}\n${value.taskSummary}`;
          await fs.default.writeFile(memoryPath, newContent.trim(), 'utf-8');
          addItem(
            {
              type: 'info',
              text: `📝 Task state saved to QWEN.md. After /clear, say "continue previous task" to resume.`,
            },
            Date.now(),
          );
        } catch {
      ...
      addItem(
        {
          type: 'error',
          text:
            `🚫 Token limit exceeded (${value.currentTokens.toLocaleString()}/${value.limit.toLocaleString()}). ` +
            `Fix: /clear, /compress, or set higher "sessionTokenLimit" in settings.json`,
        },
        Date.now(),
      );
    },
  ...
  const handleCompletedTools = useCallback(
    async (completedToolCallsFromScheduler: TrackedToolCall[]) => {
      ...
      // If all the tools were cancelled, check if any have EditRequest outcome
      // (user provided feedback). If so, we should continue with the feedback.
      const allToolsCancelled = geminiTools.every(
        (tc) => tc.status === 'cancelled',
      );

      if (allToolsCancelled) {
        // Check if any cancelled tool has EditRequest outcome (user feedback)
        const hasEditRequestWithFeedback = geminiTools.some(
          (tc) =>
            tc.status === 'cancelled' &&
            tc.outcome === ToolConfirmationOutcome.EditRequest,
        );
        ...
        if (hasEditRequestWithFeedback) {
          // User provided feedback, continue with the LLM so it can see the feedback
          ...
          // Submit the tool responses including feedback to continue the conversation
          submitQuery(
            combinedParts,
            {
              isContinuation: true,
        ...
        // Simple cancel without feedback - just add to history
        if (geminiClient) {
