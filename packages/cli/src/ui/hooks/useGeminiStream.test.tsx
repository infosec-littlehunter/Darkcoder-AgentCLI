...
vi.mock('@darkcoder/darkcoder-core', async (importOriginal) => {
...
describe('useGeminiStream', () => {
  ...
  it('should continue conversation when tool is cancelled with EditRequest feedback', async () => {
    const cancelledWithFeedback: TrackedCancelledToolCall = {
      request: {
        callId: 'edit-request-1',
        name: 'run_shell_command',
        args: { command: 'rm -rf /' },
        isClientInitiated: false,
        prompt_id: 'prompt-id-edit',
      },
      status: 'cancelled',
      outcome: ToolConfirmationOutcome.EditRequest,
      response: {
        callId: 'edit-request-1',
        responseParts: [
          {
            functionResponse: {
              ...
              response: {
                error:
                  '[Operation Cancelled] Reason: Please use a safer command',
              },
            },
          },
        ],
      ...
      invocation: {
        getDescription: () => 'Run shell command',
      } as unknown as AnyToolInvocation,
    } as TrackedCancelledToolCall;
    ...
    // Trigger the onComplete callback with EditRequest cancelled tool
    await act(async () => {
      if (capturedOnComplete) {
        await capturedOnComplete([cancelledWithFeedback]);
      }
    });

    await waitFor(() => {
      // Tools should be marked as submitted
      expect(mockMarkToolsAsSubmitted).toHaveBeenCalledWith(['edit-request-1']);

      // Unlike simple cancellation, we SHOULD call sendMessageStream to continue
      // the conversation with the user's feedback
      expect(mockSendMessageStream).toHaveBeenCalledWith(
        cancelledWithFeedback.response.responseParts,
        expect.any(Object), // AbortSignal
        'prompt-id-edit',
        { isContinuation: true },
      );

      // addHistory should NOT be called (submitQuery handles this internally)
