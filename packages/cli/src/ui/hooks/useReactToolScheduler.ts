...
import { CoreToolScheduler } from '@darkcoder/darkcoder-core';
...
export function useReactToolScheduler(
  ...
  // Note: outputUpdateHandler is now a no-op for the interactive UI.
  // Live output updates are handled through the throttled toolCallsUpdateHandler
  // to prevent excessive re-renders during streaming. The core scheduler still
  // calls this, but we ignore it since toolCallsUpdateHandler gets the same data.
  const outputUpdateHandler: OutputUpdateHandler = useCallback(
    (_toolCallId, _outputChunk) => {
      // Intentionally empty - updates come via throttled toolCallsUpdateHandler
    },
    [],
  );
      ...
      setToolCallsForDisplay((prevTrackedCalls) =>
        updatedCoreToolCalls.map((coreTc) => {
          ...
          if (coreTc.status === 'executing') {
            const coreExecuting = coreTc as ExecutingToolCall;
            return {
              ...coreTc,
              responseSubmittedToGemini,
              // Use liveOutput from core (throttled updates) - this is the source of truth
              liveOutput: coreExecuting.liveOutput,
              pid: coreExecuting.pid,
            };
          }
  ...
  // The scheduler has internal resources that need cleanup
  useEffect(() => () => {
      scheduler.dispose();
    }, [scheduler]);
