...
/**
 * Check if any API key env var is set that would allow authentication
 */
function hasApiKeyEnvVar(): boolean {
  return !!(
...
export const useAuthCommand = (
  ...
  // Check if authenticated via settings OR via environment variable
  const hasEnvApiKey = hasApiKeyEnvVar();
  const unAuthenticated =
    settings.merged.security?.auth?.selectedType === undefined && !hasEnvApiKey;
