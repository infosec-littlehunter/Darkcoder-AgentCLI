...
export function AuthDialog(): React.JSX.Element {
  ...
  // Special value to indicate opening provider keys dialog
  const MANAGE_KEYS = 'manage_keys' as AuthType;
  ...
  const items = [
    ...
    {
      key: MANAGE_KEYS,
      label: t('🔑 Manage Provider API Keys'),
      value: MANAGE_KEYS,
    },
  ];
  ...
  const { openProviderKeysDialog } = useUIActions();

  const handleAuthSelect = async (authMethod: AuthType) => {
    setErrorMessage(null);
    // Handle special case for managing provider keys
    if (authMethod === MANAGE_KEYS) {
