...
import type { Config } from '@darkcoder/darkcoder-core';
import {
  AuthType,
  getOauthClient,
  InputFormat,
...
function getNodeMemoryArgs(isDebugMode: boolean): string[] {
  ...
  // Set target to 60% of total memory, with a minimum of 4GB and maximum of 8GB
  // This provides more headroom for long-running sessions while preventing
  // excessive memory usage on high-memory systems
  const calculatedMemory = Math.floor(totalMemoryMB * 0.6);
  const targetMaxOldSpaceSizeInMB = Math.max(
    4096, // Minimum 4GB
    Math.min(calculatedMemory, 8192), // Maximum 8GB
  );
  if (isDebugMode) {
    console.debug(
...
export async function startInteractiveUI(
  ...
  // This helps prevent OOM crashes by triggering GC when memory is critical
  const isDebugMode = config.getDebugMode();
  startMemoryMonitor({
    ...
    onWarning: (msg) => {
      if (isDebugMode) {
        console.warn(`[Memory Warning] ${msg}`);
      }
    },
    onCritical: (msg) => {
      console.error(`[Memory Critical] ${msg}`);
      console.error(
        'Consider restarting the session or increasing NODE_OPTIONS heap limit',
      );
    },
  });
  ...
  registerCleanup(() => {
    if (isDebugMode) {
      console.log(`[Memory] Final usage: ${formatMemoryUsage()}`);
    }
    stopMemoryMonitor();
  });
