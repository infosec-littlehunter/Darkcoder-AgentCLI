 ...
 * and suggestions will be provided if available.
 *
 ...
 * @returns An object containing the resolved command, its arguments, canonical path, and optional suggestions.
 */
export const parseSlashCommand = (
  ...
  for (const part of commandPath) {
    ...
    if (foundCommand) {
      ...
      // Command not found - generate suggestions
      const availableCommands = currentCommands.map((cmd) => ({
        name: cmd.name,
        description: cmd.description,
      }));

      const suggestions = findSlashCommandSuggestions(part, availableCommands);
      const suggestionMessage =
        suggestions.length > 0
          ? formatSuggestions(part, suggestions, true)
          : undefined;
