...
Note: This SDK is vendored in the DarkCoder monorepo. Within DarkCoder, it enables programmatic control and MCP interop; most users should use the DarkCoder CLI directly. The upstream package name and API remain unchanged for compatibility.

A minimum experimental TypeScript SDK for programmatic access to Qwen Code (and compatible CLIs like DarkCoder).
...
- Node.js >= 20.0.0
- [Qwen Code](https://github.com/QwenLM/qwen-code) >= 0.4.0 (stable) or DarkCoder CLI installed and accessible in PATH

> **Note for nvm users**: If you use nvm to manage Node.js versions, the SDK may not be able to auto-detect the Qwen Code executable. You should explicitly set the `pathToQwenExecutable` option to the full path of the `qwen` binary. When using DarkCoder, set this option to the path of the `darkcoder` executable.
