...
export class TextTokenizer {
  ...
  async calculateTokens(text: string): Promise<number> {
    ...
    if (this.encoding) {
      try {
        // Allow all special tokens to avoid errors with <|endoftext|> etc.
        return this.encoding.encode(text, 'all').length;
      } catch (error) {
    ...
    if (this.encoding) {
      try {
        return texts.map((text) => {
          ...
          // Allow all special tokens to avoid errors with <|endoftext|> etc.
          return this.encoding ? this.encoding.encode(text, 'all').length : 0;
        });
      } catch (error) {
