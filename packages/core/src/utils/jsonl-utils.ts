 ...
 * Limited size to prevent memory leaks during long sessions.
 */
const fileLocks = new Map<string, Mutex>();
const MAX_FILE_LOCKS = 1000;

/**
 * Gets or creates a mutex for a specific file path.
 * Cleans up old locks when cache is too large.
 */
function getFileLock(filePath: string): Mutex {
  if (!fileLocks.has(filePath)) {
    // Evict oldest entries if cache is too large
    if (fileLocks.size >= MAX_FILE_LOCKS) {
      const keysToDelete = Array.from(fileLocks.keys()).slice(
        0,
        Math.floor(MAX_FILE_LOCKS / 4),
      );
      for (const key of keysToDelete) {
        const mutex = fileLocks.get(key);
        // Only delete if mutex is not currently locked
