...
export class SubagentStatistics {
  ...
  getSummary(now = Date.now()): SubagentStatsSummary {
    ...
    // Updated cost calculation with more accurate pricing (average across models)
    // Input: ~$3/1M tokens, Output: ~$15/1M tokens (based on Claude Sonnet 4.5 pricing)
    const estimatedCost =
      (this.inputTokens / 1_000_000) * 3.0 +
      (this.outputTokens / 1_000_000) * 15.0;
    return {
      rounds: this.rounds,
      totalDurationMs,
      totalToolCalls,
