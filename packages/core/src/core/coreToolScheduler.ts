...
export class CoreToolScheduler {
  ...
  // Performance: UI update throttler to prevent excessive re-renders (max 20/second)
  ...
  constructor(options: CoreToolSchedulerOptions) {
    ...
    // Initialize UI throttler to cap updates at ~20/second
  ...
  private setStatusInternal(
    ...
    // All status changes are immediate - they happen infrequently
    // Performance throttling is applied only to liveOutput updates (see below)
    this.uiThrottler.forceNotification();
  ...
  private notifyToolCallsUpdate(): void {
    // Use throttled notification to prevent UI thrashing during streaming
    this.uiThrottler.requestNotification();
  }

  // Actual notification method called by throttler
   ...
   * to prevent memory leaks from timers and other resources.
   */
  dispose(): void {
    // Clear any pending tool calls and request queue
    this.toolCalls = [];
    this.requestQueue = [];

    // Dispose UI throttler to clear pending timers
