...
export interface SessionTokenLimitExceededValue {
  ...
  limit: number;
  message: string;
  /** Auto-generated task summary for continuation in new session */
  taskSummary?: string;
}
...
export class Turn {
  ...
  /**
   * Helper to check if an error is a stream termination error that should be retried
   */
  private isRetryableStreamError(error: unknown): boolean {
    if (error instanceof Error) {
      const message = error.message.toLowerCase();
      // Network termination errors
      if (
  ...
  async *run(
    ...
    while (retryCount <= Turn.MAX_STREAM_RETRIES) {
      try {
        ...
        if (
          this.isRetryableStreamError(e) &&
          retryCount < Turn.MAX_STREAM_RETRIES
        ) {
          retryCount++;
          const delayMs = Turn.RETRY_DELAY_MS * retryCount; // Exponential backoff

          // Yield a retry event so the UI knows we're retrying
          yield { type: GeminiEventType.Retry };

          // Log the retry attempt (will be in error reports)
          console.error(
            `Stream terminated, retrying (${retryCount}/${Turn.MAX_STREAM_RETRIES}) in ${delayMs}ms...`,
          );
