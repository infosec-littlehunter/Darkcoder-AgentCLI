...
export function determineProvider(
  ...
  // Check for Anthropic/Claude provider
  ...
  // Check for Google Gemini provider
  if (GeminiOpenAICompatibleProvider.isGeminiProvider(config)) {
    return new GeminiOpenAICompatibleProvider(
      contentGeneratorConfig,
      cliConfig,
    );
  }
