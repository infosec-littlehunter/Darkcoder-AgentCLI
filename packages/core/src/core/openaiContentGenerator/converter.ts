...
export class OpenAIContentConverter {
  ...
  private cleanOrphanedToolCalls(
    ...
    // Ensure tool responses immediately follow their triggering assistant tool_calls
    const adjacencyCleaned: OpenAI.Chat.ChatCompletionMessageParam[] = [];
    let pendingToolCallIds = new Set<string>();

    for (const message of finalCleaned) {
      if (
        message.role === 'assistant' &&
        'tool_calls' in message &&
        message.tool_calls
        ...
        // Any non-tool message breaks the immediate tool-call chain
        pendingToolCallIds = new Set();
        adjacencyCleaned.push(message);
      }
    }

    return adjacencyCleaned;
  }
