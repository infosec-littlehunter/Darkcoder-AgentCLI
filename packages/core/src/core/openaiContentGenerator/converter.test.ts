...
describe('OpenAIContentConverter', () => {
  ...
  describe('cleanOrphanedToolCalls adjacency guard', () => {
    it('should drop tool responses that are not immediately after their tool_calls', () => {
      const messages: OpenAI.Chat.ChatCompletionMessageParam[] = [
        ...
        { role: 'user', content: 'interrupting user message' },
        {
          role: 'tool',
          tool_call_id: 'call_1',
          content: 'should be dropped because not adjacent',
        },
      ];
      ...
      expect(cleaned.find((m) => m.role === 'tool')).toBeUndefined();
    });

    it('should keep tool responses that directly follow their tool_calls', () => {
      const messages: OpenAI.Chat.ChatCompletionMessageParam[] = [
        {
          role: 'assistant',
          content: null,
      ...
      const toolMessage = cleaned.find((m) => m.role === 'tool');
      expect(toolMessage).toBeDefined();
      expect(
        (toolMessage as OpenAI.Chat.ChatCompletionMessageParam).content,
      ).toBe('kept');
    });
  });
});
