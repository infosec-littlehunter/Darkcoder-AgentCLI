...
import { VirusTotalTool } from '../tools/virustotal.js';
...
export interface ConfigParameters {
  ...
  // OSINT API keys for security tools
  censysApiKey?: string;
  censysApiId?: string;
...
export class Config {
  ...
  // OSINT API keys
  private readonly censysApiKey?: string;
  private readonly censysApiId?: string;
  private readonly censysApiSecret?: string;
  private readonly urlscanApiKey?: string;
  private readonly cuckooApiUrl?: string;
  private readonly cuckooApiToken?: string;
  private readonly virusTotalApiKey?: string;
  private readonly yaraifyApiKey?: string;
  private readonly hybridAnalysisApiKey?: string;
  // Response optimization
  private readonly conciseMode: boolean;
  ...
  constructor(params: ConfigParameters) {
    ...
    // OSINT API keys
    this.censysApiKey = params.censysApiKey;
    this.censysApiId = params.censysApiId;
    this.censysApiSecret = params.censysApiSecret;
    this.urlscanApiKey = params.urlscanApiKey;
    this.cuckooApiUrl = params.cuckooApiUrl;
    this.cuckooApiToken = params.cuckooApiToken;
    this.virusTotalApiKey = params.virusTotalApiKey;
    this.yaraifyApiKey = params.yaraifyApiKey;
    this.hybridAnalysisApiKey = params.hybridAnalysisApiKey;
    // Response optimization
    this.conciseMode = params.conciseMode ?? false;
  ...
  async refreshAuth(authMethod: AuthType, isInitialAuth?: boolean) {
    ...
    // Reset the chat to use the new model and content generator
    // This ensures the chat uses the correct system prompt and model after auth change
    if (this.geminiClient && !isInitialAuth) {
  ...
  // OSINT API keys
  getCensysApiKey(): string | undefined {
    return this.censysApiKey || process.env['CENSYS_API_KEY'];
  }

  getCensysApiId(): string | undefined {
    return this.censysApiId || process.env['CENSYS_API_ID'];
  }

  getCensysApiSecret(): string | undefined {
    return this.censysApiSecret || process.env['CENSYS_API_SECRET'];
  }

  getUrlscanApiKey(): string | undefined {
    return this.urlscanApiKey || process.env['URLSCAN_API_KEY'];
  }

  getCuckooApiUrl(): string | undefined {
    return this.cuckooApiUrl || process.env['CUCKOO_API_URL'];
  }

  getCuckooApiToken(): string | undefined {
    return this.cuckooApiToken || process.env['CUCKOO_API_TOKEN'];
  }

  getVirusTotalApiKey(): string | undefined {
    return this.virusTotalApiKey || process.env['VIRUSTOTAL_API_KEY'];
  }

  getYaraifyApiKey(): string | undefined {
    return this.yaraifyApiKey || process.env['YARAIFY_API_KEY'];
  }

  getHybridAnalysisApiKey(): string | undefined {
    return this.hybridAnalysisApiKey || process.env['HYBRID_ANALYSIS_API_KEY'];
  }

  /**
   * Returns true if concise mode is enabled.
   * When enabled, the LLM should provide shorter, more focused responses
   * without unsolicited suggestions or explanations.
   */
  getConciseMode(): boolean {
  ...
  async createToolRegistry(
    ...
    // OSINT tools for red team/bug bounty reconnaissance
    // These tools are always registered but require API keys to function
    registerCoreTool(WaybackMachineTool, this);
    ...
    registerCoreTool(ToolValidationTool, this);

    // Malware analysis sandbox - self-hosted
    registerCoreTool(CuckooSandboxTool, this);

    // YARA rule scanning and validation with YARAify
    registerCoreTool(YaraifyTool, this);

    // VirusTotal threat intelligence
    registerCoreTool(VirusTotalTool, this);

    // Hybrid Analysis - CrowdStrike advanced malware sandbox analysis
    // Dynamic behavioral analysis, MITRE ATT&CK mapping, IOC extraction
    registerCoreTool(HybridAnalysisTool, this);

    // Security hardening and compliance tool with RAG pipeline
    // Provides CIS Benchmarks, Microsoft Security docs, and compliance guidance
    registerCoreTool(CISBenchmarkTool, this);
