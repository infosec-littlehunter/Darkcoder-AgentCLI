      ...
      const result = new Promise<ShellExecutionResult>((resolve) => {
        ...
        const render = (finalRender = false) => {
          ...
          if (finalRender) {
            ...
            // Use 50ms throttle (~20fps) to reduce terminal shaking during high CPU usage
            // Previously 17ms (~60fps) caused excessive re-renders and UI instability
            renderTimeout = setTimeout(renderFn, 50);
          }
        };
