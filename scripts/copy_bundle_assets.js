...
// Copy system prompt markdown files
const promptFiles = [
  'expert-ai-system-prompt.md',
  'optimized-ai-system-prompt.md',
];
...
console.log('Copied system prompt files to dist/');

// Copy Python animated banner script
const scriptsDistDir = join(distDir, 'scripts');
if (!existsSync(scriptsDistDir)) {
  mkdirSync(scriptsDistDir, { recursive: true });
}
const bannerScriptSrc = join(root, 'scripts', 'animated-banner.py');
const bannerScriptDest = join(scriptsDistDir, 'animated-banner.py');
if (existsSync(bannerScriptSrc)) {
  copyFileSync(bannerScriptSrc, bannerScriptDest);
  console.log('Copied animated banner script to dist/scripts/');
}
